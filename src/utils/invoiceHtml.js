...
const PDF_STYLES = `
  ...
  * { box-sizing: border-box; }
  ...
  /* --------------------------------------------------------------------
   * iOS / mobile Safari phone-number auto-linking fix
   * --------------------------------------------------------------------
   * When this invoice HTML is printed/saved to PDF from an iPhone,
   * WebKit's "data detectors" scan the rendered text and silently wrap
   * anything that looks like a phone number in an invisible
   * <a href="tel:...">, styled with Safari's own deep-blue link color and
   * underline. This happens entirely inside iOS's renderer — nothing in
   * this file creates a real <a> tag around phone numbers — which is why
   * desktop browsers and Android (neither of which auto-links phone
   * numbers this way) are completely unaffected either way.
   *
   * Two layers of defense, matching the two ways this can happen:
   *   1. The <meta name="format-detection" content="telephone=no" /> tag
   *      in <head> (added in the HTML output below) tells iOS not to do
   *      this in the first place. This alone is sufficient in current
   *      Mobile Safari.
   *   2. This CSS is a defensive fallback for any WebKit build that
   *      still auto-links despite the meta tag (older iOS versions are
   *      known to sometimes ignore it). It only targets the auto-detector
   *      link styles and forces them back to the surrounding text's own
   *      color/weight/decoration — it does not touch the phone number
   *      text, remove any link, or change layout/spacing in any way.
   * Neither layer removes the phone numbers or makes them non-functional
   * anywhere else in the app; this fix is scoped to the generated
   * invoice/PDF HTML documents only.
   */
  ...
  /* Customer phone number (Bill To card) keeps its existing gray. */
  .customer-line a,
  .customer-line a[x-apple-data-detectors],
  .customer-line a[href^="tel"] {
    color: #374151 !important;
    text-decoration: none !important;
    font: inherit !important;
  }
  /* Shop phone number (header band) keeps its existing translucent white
     — the same equivalent protection as the customer line above, applied
     to .shop-meta since that is the class the shop phone/address use. */
  .shop-meta a,
  .shop-meta a[x-apple-data-detectors],
  .shop-meta a[href^="tel"] {
...
export function buildSaleInvoiceHtml({ shop, sale, customer, lineItems, payments }) {
  ...
  return `
    <!DOCTYPE html>
    <html>
      <head><meta charset="utf-8" /><meta name="format-detection" content="telephone=no" /><style>${PDF_STYLES}</style></head>
      <body>
        ${header}
        <div class="content">
          <div class="meta-row">
